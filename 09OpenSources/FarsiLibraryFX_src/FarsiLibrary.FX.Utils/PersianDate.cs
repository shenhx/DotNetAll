...
using FarsiLibrary.FX.Utils.Exceptions;
...
namespace FarsiLibrary.FX.Utils
{
        ...
        IEquatable<PersianDate>
    {
        ...
        public class PersianMonthNames
        {
            ...
            private static readonly PersianMonthNames def;
        ...
        public class PersianWeekDayNames
        {
            ...
            private static readonly PersianWeekDayNames def;
        ...
        public class PersianWeekDayAbbr
        {
            ...
            private static readonly PersianWeekDayAbbr def;
            ...
            private string GetName(int WeekDayNo)
            {
                switch (WeekDayNo)
                {
                    ...
                    default:
                        throw new ArgumentOutOfRangeException("WeekDayNo", "WeekDay number is out of range");
                }
            }
        ...
        private static readonly PersianDate now;
        private readonly string amDesignator = Thread.CurrentThread.CurrentCulture.DateTimeFormat.AMDesignator;
        private readonly string pmDesignator = Thread.CurrentThread.CurrentCulture.DateTimeFormat.PMDesignator;
        ...
        static PersianDate()
        {
            ...
            MinValue = new PersianDate(1, 1, 2, 12, 0, 0, 0); // 12:00:00.000 AM, 22/03/0622
        ...
        /// </summary>
        /// <param name="MillisecondNo"></param>
        ...
        {
            return (LocalizedWeekDayName + " " + day + " " + LocalizedMonthName + " " + year);
        }
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            {
                case "yyyymmdd":
                    return string.Format("{0}{1}{2}", Year, toDouble(Month), toDouble(Day));

                case "D":
                    //'yyyy mm dd dddd' e.g. 'دوشنبه 20 شهریور 1384'
                    return string.Format("{0} {1} {2} {3}", new object[4] { LocalizedWeekDayName, toDouble(Day), LocalizedMonthName, Year });

                case "f":
                    //'hh:mm yyyy mmmm dd dddd' e.g. 'دوشنبه 20 شهریور 1384 21:30'
                    return string.Format("{0} {1} {2} {3}  {4}:{5}", new object[6] { LocalizedWeekDayName, toDouble(Day), LocalizedMonthName, Year, toDouble(Hour), toDouble(Minute) });

                case "F":
                    //'tt hh:mm:ss yyyy mmmm dd dddd' e.g. 'دوشنبه 20 شهریور 1384 02:30:22 ب.ض'
                    return string.Format("{0} {1} {2} {3}  {4}:{5}:{6}  {7}", new object[8] { LocalizedWeekDayName, toDouble(Day), LocalizedMonthName, Year, toDouble(smallhour), toDouble(Minute), toDouble(Second), designator });

                case "L":
                    //'yyyy/mm/dd HH:MM'
                    return string.Format("{0}/{1}/{2}  {3}:{4}", new object[5] { Year, toDouble(Month), toDouble(Day), toDouble(hour), toDouble(minute) });

                case "g":
                    //'yyyy/mm/dd hh:mm tt'
                    return string.Format("{0}/{1}/{2}  {3}:{4} {5}", new object[6] { Year, toDouble(Month), toDouble(Day), toDouble(smallhour), toDouble(Minute), designator });

                case "G":
                    //'yyyy/mm/dd hh:mm:ss tt'
                    return string.Format("{0}/{1}/{2}  {3}:{4}:{5} {6}", new object[7] { Year, toDouble(Month), toDouble(Day), toDouble(smallhour), toDouble(Minute), toDouble(Second), designator });
                ...
                case "s":
                    //'yyyy-mm-ddThh:mm:ss'
                    return string.Format("{0}-{1}-{2}T{3}:{4}:{5}", Year, toDouble(Month), toDouble(Day), toDouble(Hour), toDouble(Minute), toDouble(Second));

                case "t":
                    //'hh:mm tt'
                    return string.Format("{0}:{1} {2}", toDouble(smallhour), toDouble(Minute), designator);

                case "T":
                    //'hh:mm:ss tt'
                    return string.Format("{0}:{1}:{2} {3}", new object[4] { toDouble(smallhour), toDouble(Minute), toDouble(Second), designator });
