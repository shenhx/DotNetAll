...
{
    public abstract class FirstAbpDemoTestBase : AbpIntegratedTestBase<FirstAbpDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FirstAbpDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<FirstAbpDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FirstAbpDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FirstAbpDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FirstAbpDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FirstAbpDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstAbpDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FirstAbpDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FirstAbpDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FirstAbpDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FirstAbpDemoDbContext, Task<T>> func)
        {
            T result;
