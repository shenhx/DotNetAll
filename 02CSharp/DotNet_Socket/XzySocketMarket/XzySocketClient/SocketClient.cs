...
namespace XzySocketClient
{
    /// <summary>
    /// socket client
    /// </summary>
    /// <typeparam name="TMessage"></typeparam>
    public class SocketClient<TMessage> : BaseHost
        where TMessage : class, Messaging.IMessage
    {
        ...
        /// </summary>
        public event Action<IConnection, TMessage> ReceivedUnknowMessage;
        #endregion
        ...
        public bool TryRegisterEndPoint(string name, EndPoint[] arrRemoteEP, Func<IConnection, Task> initFunc = null)
        {
        ...
        /// <exception cref="ArgumentNullException">request is null.</exception>
        public void Send(IConnection connection, Request<TMessage> request)
        {
        ...
        /// <param name="name"></param>
        /// <param name="connection"></param>
        protected virtual void OnEndPointConnected(string name, IConnection connection)
        {
        ...
        /// <param name="name"></param>
        /// <param name="connection"></param>
        protected virtual void OnEndPointAlready(string name, IConnection connection)
        {
        ...
        protected virtual void OnPendingSendTimeout(Request<TMessage> request)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
                ...
                catch (Exception ex) { XzySocketBase.Log.Trace.Error(ex.Message, ex); }
            });
        }
        /// <summary>
        /// on request sent
        ...
        /// <param name="connection"></param>
        /// <param name="request"></param>
        protected virtual void OnSent(IConnection connection, Request<TMessage> request)
        {
        }
        /// <summary>
        /// on send failed
        ...
        protected virtual void OnReceived(IConnection connection, Request<TMessage> request, TMessage message)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
        ...
        /// <param name="connection"></param>
        /// <param name="message"></param>
        protected virtual void OnReceivedUnknowMessage(IConnection connection, TMessage message)
        {
        ...
        protected override void OnSendCallback(IConnection connection, Packet packet, bool isSuccess)
        {
            ...
            if (isSuccess)
            {
                request.SentTime = XzySocketBase.Utils.Date.UtcNow;
            ...
            if (XzySocketBase.Utils.Date.UtcNow.Subtract(request.CreatedTime).TotalMilliseconds > this._millisecondsSendTimeout)
            {
                //send time out
        ...
        private class PendingSendQueue
        {
            ...
            public PendingSendQueue(SocketClient<TMessage> client)
            {
                ...
                this._timer = new Timer(state =>
                {
                    ...
                    var dtNow = XzySocketBase.Utils.Date.UtcNow;
        ...
        private class NodeInfo
        {
            ...
            /// init function
            /// </summary>
            public readonly Func<IConnection, Task> InitFunc;
            #endregion
            ...
            public NodeInfo(string name, EndPoint[] arrRemoteEP,
                Func<IConnection, Task> initFunc)
            {
        ...
        private class Node : IDisposable
        {
            ...
            private readonly Action<Node, IConnection> _connectedCallback;
            private readonly Action<Node, IConnection> _alreadyCallback;
            ...
            public Node(NodeInfo info, IHost host,
                Action<Node, IConnection> connectedCallback,
                Action<Node, IConnection> alreadyCallback)
            {
            ...
            private void ConnectCallback(Task<Socket> t)
            {
                if (t.IsFaulted)
                {
                    ...
                    XzySocketBase.Utils.TaskEx.Delay(new Random().Next(500, 1500)).ContinueWith(_ => this.Connect());
                    return;
                }
                ...
                connection.Disconnected += (conn, ex) =>
                {
                    ...
                    XzySocketBase.Utils.TaskEx.Delay(new Random().Next(100, 1000)).ContinueWith(_ => this.Connect());
                };
        ...
        private class EndPointManager
        {
            ...
            /// node already event
            /// </summary>
            public event Action<string, IConnection> Already;
            #endregion
            ...
            /// </summary>
            /// <param name="host"></param>
            public EndPointManager(IHost host)
            {
                this._host = host;
            }
            #endregion
            ...
            /// <param name="initFunc"></param>
            /// <returns></returns>
            public bool TryRegister(string name, EndPoint[] arrRemoteEP, Func<IConnection, Task> initFunc)
            {
                lock (this)
                {
            ...
            /// <param name="connection"></param>
            private void OnNodeConnected(Node node, IConnection connection)
            {
            ...
            /// <param name="node"></param>
            /// <param name="connection"></param>
            private void OnNodeAlready(Node node, IConnection connection)
            {
        ...
        private interface IConnectionPool
        {
            ...
            /// <param name="connection"></param>
            /// <returns></returns>
            bool TryAcquire(out IConnection connection);
            /// <summary>
            /// release
            /// </summary>
            /// <param name="connection"></param>
        ...
        public sealed class AsyncPool : IConnectionPool
        {
            ...
            /// </summary>
            /// <param name="connection"></param>
            public void Destroy(IConnection connection)
            {
                if (connection == null) throw new ArgumentNullException("connection");
        ...
        public sealed class SyncPool : IConnectionPool
        {
            ...
            private readonly ConcurrentStack<IConnection> _stack =
